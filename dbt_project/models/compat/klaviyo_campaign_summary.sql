...
select
    ...
    m.recipients,
    m.opens,
    safe_divide(m.opens, nullif(cast(m.recipients as float64), 0)) as open_rate,
    m.clicks,
    safe_divide(m.clicks, nullif(cast(m.recipients as float64), 0)) as click_rate,
    m.revenue,
    safe_divide(m.revenue, nullif(cast(m.recipients as float64), 0)) as revenue_per_recipient,
    m.ingested_at
